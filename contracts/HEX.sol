...
contract GlobalsAndUtility is ERC20 {
    ...
    */
    ...
    /*  Claim             (auto-generated event)
        ...
        uint72            claimedABET   -->  data0 [231:160]
        address  indexed  referrerAddr
    ...
    */
    ...
    /*  StakeStart        (auto-generated event)
        ...
        uint72            stakedABET    -->  data0 [111: 40]
    ...
    */
    ...
    /* Seed for WAAS Lobby */
    uint256 internal constant WAAS_LOBBY_SEED_FIRE = 1e9;
    uint256 internal constant WAAS_LOBBY_SEED_ABET = WAAS_LOBBY_SEED_FIRE * ABET_PER_FIRE;
    ...
    /* Total Satoshis from all ABET addresses in UTXO snapshot */
    uint256 internal constant FULL_SATOSHIS_TOTAL = 1807766732160668;

    /* Total Satoshis from supported ABET addresses in UTXO snapshot after applying Silly Whale */
    ...
    /* Stake shares Longer Pays Better bonus constants used by _stakeStartBonusABET() */
    ...
    /* Stake shares Bigger Pays Better bonus constants used by _stakeStartBonusABET() */
    uint256 private constant BPB_BONUS_PERCENT = 10;
    uint256 private constant BPB_MAX_FIRE = 150 * 1e6;
    uint256 internal constant BPB_MAX_ABET = BPB_MAX_FIRE * ABET_PER_FIRE;
    uint256 internal constant BPB = BPB_MAX_ABET * 100 / BPB_BONUS_PERCENT;
    ...
    uint8 internal constant ETH_ADDRESS_FIRE_LEN = ETH_ADDRESS_BYTE_LEN * 2;

    uint8 internal constant CLAIM_PARAM_HASH_BYTE_LEN = 12;
    uint8 internal constant CLAIM_PARAM_HASH_FIRE_LEN = CLAIM_PARAM_HASH_BYTE_LEN * 2;

    uint8 internal constant BITCOIN_SIG_PREFIX_LEN = 24;
    bytes24 internal constant BITCOIN_SIG_PREFIX_STR = "Bitcoin Signed Message:\n";

    bytes internal constant STD_CLAIM_PREFIX_STR = "Claim_FIRE_to_0x";
    bytes internal constant OLD_CLAIM_PREFIX_STR = "Claim_BitcoinFIRE_to_0x";

    bytes16 internal constant FIRE_DIGITS = "0123456789abcdef";

    /* Claim flags passed to abetAddressClaim()  */
    uint8 internal constant CLAIM_FLAG_MSG_PREFIX_OLD = 1 << 0;
    uint8 internal constant CLAIM_FLAG_ABET_ADDR_COMPRESSED = 1 << 1;
    uint8 internal constant CLAIM_FLAG_ABET_ADDR_P2WPKH_IN_P2SH = 1 << 2;
    uint8 internal constant CLAIM_FLAG_ABET_ADDR_BECH32 = 1 << 3;
    uint8 internal constant CLAIM_FLAG_ETH_ADDR_LOWERCASE = 1 << 4;

    /* Globals expanded for memory (except _latestStakeId) and compact for storage */
    struct GlobalsCache {
        // 1
        uint256 _lockedABETTotal;
        ...
        external
    {
        ...
        require(g._currentDay > CLAIM_PHASE_START_DAY, "FIRE: Too early");

        if (beforeDay != 0) {
            require(beforeDay <= g._currentDay, "FIRE: beforeDay cannot be in the future");
    ...
    {
        require(beginDay < endDay && endDay <= globals.dailyDataCount, "FIRE: range invalid");
        ...
        uint256 v;
        do {
            v = uint256(dailyData[src].dayUnclaimedSatoshisTotal) << (ABET_UINT_SIZE * 2);
            v |= uint256(dailyData[src].dayStakeSharesTotal) << ABET_UINT_SIZE;
        ...
        returns (uint256[13] memory)
    {
        ...
        (_claimedAbetAddrCount, _claimedSatoshisTotal, _unclaimedSatoshisTotal) = _claimStatsDecode(
            globals.claimStats
        );

        return [
            // 1
            globals.lockedABETTotal,
     ...
     * staked ABET. allocatedSupply() includes both.
     * @return Allocated Supply in ABET
     */
    function allocatedSupply()
        ...
        returns (uint256)
    {
        return totalSupply() + globals.lockedABETTotal;
    }
    ...
    {
        // 1
        g._lockedABETTotal = globals.lockedABETTotal;
        ...
        (g._claimedAbetAddrCount, g._claimedSatoshisTotal, g._unclaimedSatoshisTotal) = _claimStatsDecode(
            globals.claimStats
        );
        //
    ...
    {
        // 1
        gSnapshot._lockedABETTotal = g._lockedABETTotal;
        ...
        view
    {
        ...
        require(stakeIdParam == stRef.stakeId, "FIRE: stakeIdParam not in stake");

        st._stakeId = stRef.stakeId;
        st._stakedABET = stRef.stakedABET;
        ...
        internal
    {
        ...
        stRef.stakedABET = uint72(st._stakedABET);
    ...
    function _claimStatsEncode(
        ...
        returns (uint128)
    {
        uint256 v = _claimedAbetAddrCount << (SATOSHI_UINT_SIZE * 2);
    ...
    function _claimStatsDecode(uint128 v)
        internal
        pure
        returns (uint256 _claimedAbetAddrCount, uint256 _claimedSatoshisTotal, uint256 _unclaimedSatoshisTotal)
    {
        _claimedAbetAddrCount = v >> (SATOSHI_UINT_SIZE * 2);
        _claimedSatoshisTotal = (v >> SATOSHI_UINT_SIZE) & SATOSHI_UINT_MASK;
        _unclaimedSatoshisTotal = v & SATOSHI_UINT_MASK;

        return (_claimedAbetAddrCount, _claimedSatoshisTotal, _unclaimedSatoshisTotal);
    }
        ...
        returns (uint256 payout)
    {
        ...
        rs._allocSupplyCached = totalSupply() + g._lockedABETTotal;
        ...
        if (day == BIG_PAY_DAY) {
            uint256 bigPaySlice = gTmp._unclaimedSatoshisTotal * ABET_PER_SATOSHI * stakeSharesParam
        ...
        returns (uint256)
    {
        /*
            ...
            viral = payout * (claimedAbetAddrCount / CLAIMABLE_ABET_ADDR_COUNT)
        */
        uint256 viral = payout * g._claimedAbetAddrCount / CLAIMABLE_ABET_ADDR_COUNT;
        ...
        pure
    {
        ...
        */
        ...
        if (day < CLAIM_PHASE_END_DAY) {
            uint256 bigPaySlice = g._unclaimedSatoshisTotal * ABET_PER_SATOSHI / CLAIM_PHASE_DAYS;
...
contract StakeableToken is GlobalsAndUtility {
     ...
     * @param newStakedABET Number of ABET to stake
     * @param newStakedDays Number of days to stake
     */
    function stakeStart(uint256 newStakedABET, uint256 newStakedDays)
        external
    {
        GlobalsCache memory g;
        GlobalsCache memory gSnapshot;
        ...
        require(newStakedDays >= MIN_STAKE_DAYS, "FIRE: newStakedDays lower than minimum");

        /* Check if log data needs to be updated */
        _dailyDataUpdateAuto(g);

        _stakeStart(g, newStakedABET, newStakedDays, false);

        /* Remove staked ABET from balance of staker */
        _burn(msg.sender, newStakedABET);
        ...
        external
    {
        ...
        require(stakeLists[stakerAddr].length != 0, "FIRE: Empty stake list");
        require(stakeIndex < stakeLists[stakerAddr].length, "FIRE: stakeIndex invalid");
        ...
        require(g._currentDay >= st._lockedDay + st._stakedDays, "FIRE: Stake not fully served");

        /* Stake must still be locked */
        require(st._unlockedDay == 0, "FIRE: Stake already unlocked");
        ...
        external
    {
        ...
        /* require() is more informative than the default assert() */
        require(stakeListRef.length != 0, "FIRE: Empty stake list");
        require(stakeIndex < stakeListRef.length, "FIRE: stakeIndex invalid");
            ...
            if (prevUnlocked) {
                ...
                if (servedDays > st._stakedDays) {
                    ...
                    if (servedDays < MIN_AUTO_STAKE_DAYS) {
                        require(!st._isAutoStake, "FIRE: Auto-stake still locked");
                    }
                }
            }
        ...
        g._lockedABETTotal -= st._stakedABET;
        ...
        internal
    {
        ...
        require(newStakedDays <= MAX_STAKE_DAYS, "FIRE: newStakedDays higher than maximum");

        uint256 bonusABET = _stakeStartBonusABET(newStakedABET, newStakedDays);
        uint256 newStakeShares = (newStakedABET + bonusABET) * SHARE_RATE_SCALE / g._shareRate;

        /* Ensure newStakedABET is enough for at least one stake share */
        require(newStakeShares != 0, "FIRE: newStakedABET must be at least minimum shareRate");
        ...
        */
        ...
        _emitStakeStart(newStakeId, newStakedABET, newStakeShares, newStakedDays, newAutoStake);

        /* Stake is added to total in the next round, not the current round */
        g._nextStakeSharesTotal += newStakeShares;

        /* Track total staked ABET for inflation calculations */
        g._lockedABETTotal += newStakedABET;
    }
        ...
        returns (uint256 payout)
    {
        ...
        if (beginDay <= BIG_PAY_DAY && endDay > BIG_PAY_DAY) {
            uint256 bigPaySlice = g._unclaimedSatoshisTotal * ABET_PER_SATOSHI * stakeSharesParam
    ...
    /**
     * @dev Calculate bonus ABET for a new stake, if any
     * @param newStakedABET Number of ABET to stake
     * @param newStakedDays Number of days to stake
     */
    function _stakeStartBonusABET(uint256 newStakedABET, uint256 newStakedDays)
        private
        pure
        returns (uint256 bonusABET)
    {
        /*
            LONGER PAYS BETTER:
            ...
            Bonus percentage scaled 0% to 10% for the first 150M FIRE of stake.

            biggerBonus%    = (cappedABET /  BPB_MAX_ABET) * 10%
                            = (cappedABET /  BPB_MAX_ABET) / 10
                            =  cappedABET / (BPB_MAX_ABET * 10)
                            =  cappedABET /  BPB
            ...
            combinedBonus%  =            longerBonus%  +  biggerBonus%
                                      ...
                                      cappedExtraDays     cappedABET
                            =         ---------------  +  ------------
                                            LPB               BPB

                                cappedExtraDays * BPB     cappedABET * LPB
                            =   ---------------------  +  ------------------
                                      LPB * BPB               LPB * BPB
            ...
            bonusABET     = ABET * combinedBonus%
                            = ABET * (cappedExtraDays * BPB  +  cappedABET * LPB) / (LPB * BPB)
        */
        ...
        uint256 cappedStakedABET = newStakedABET <= BPB_MAX_ABET
            ? newStakedABET
            : BPB_MAX_ABET;

        bonusABET = cappedExtraDays * BPB + cappedStakedABET * LPB;
        bonusABET = newStakedABET * bonusABET / (LPB * BPB);
        ...
        returns (uint256 stakeReturn, uint256 payout, uint256 penalty, uint256 cappedPenalty)
    {
        if (servedDays < st._stakedDays) {
            ...
            stakeReturn = st._stakedABET + payout;
        } else {
        ...
        private
    {
        if (stakeReturn > st._stakedABET) {
            ...
            */
            uint256 bonusABET = _stakeStartBonusABET(stakeReturn, st._stakedDays);
            uint256 newShareRate = (stakeReturn + bonusABET) * SHARE_RATE_SCALE / st._stakeShares;
...
contract UTXOClaimValidation is StakeableToken {
     ...
     * @param abetAddr Bitcoin address (binary; no base58-check encoding)
        ...
        returns (bool)
    {
        ...
        require(day >= CLAIM_PHASE_START_DAY, "FIRE: Claim phase has not yet started");
        require(day < CLAIM_PHASE_END_DAY, "FIRE: Claim phase has ended");

        /* Don't need to check Merkle proof if UTXO ABET address has already been claimed    */
        if (abetAddressClaims[abetAddr]) {
            return false;
        }

        /* Verify the Merkle tree proof */
        return _abetAddressIsValid(abetAddr, rawSatoshis, proof);
    }
    ...
    function claimMessageMatchesSignature(
        ...
        returns (bool)
    {
        require(v >= 27 && v <= 30, "FIRE: v invalid");
        ...
        returns (bytes20)
    {
        ...
        */
        uint8 startingByte;
        bytes memory pubKey;
        bool compressed = (claimFlags & CLAIM_FLAG_ABET_ADDR_COMPRESSED) != 0;
        bool nested = (claimFlags & CLAIM_FLAG_ABET_ADDR_P2WPKH_IN_P2SH) != 0;
        bool bech32 = (claimFlags & CLAIM_FLAG_ABET_ADDR_BECH32) != 0;

        if (compressed) {
            /* Compressed public key format */
            require(!(nested && bech32), "FIRE: claimFlags invalid");
        ...
        if (nested) {
            return _hash160(abi.encodePacked(FIRE"0014", pubKeyHash));
        }
        return pubKeyHash;
    }

    /**
     * @dev Verify a ABET address and balance are part of the Merkle tree
     * @param abetAddr Bitcoin address (binary; no base58-check encoding)
     * @param rawSatoshis Raw ABET address balance in Satoshis
     * @param proof Merkle tree proof
     * @return True if valid
     */
    function _abetAddressIsValid(bytes20 abetAddr, uint256 rawSatoshis, bytes32[] memory proof)
        internal
        pure
        returns (bool)
    {
        /*
            ...
            internal node will never have the zero-fill, as guaranteed by FIRE's Merkle
            tree construction.
            ...
            The number of leaves (CLAIMABLE_ABET_ADDR_COUNT) is even, as guaranteed by
            FIRE's Merkle tree construction, which eliminates the only edge-case where
            this validation would not apply.
        */
        require((uint256(proof[0]) & MERKLE_LEAF_FILL_MASK) == 0, "FIRE: proof invalid");
        for (uint256 i = 1; i < proof.length; i++) {
            require((uint256(proof[i]) & MERKLE_LEAF_FILL_MASK) != 0, "FIRE: proof invalid");
        }

        /*
            Calculate the 32 byte Merkle leaf associated with this ABET address and balance
                160 bits: ABET address
                 52 bits: Zero-fill
                 45 bits: Satoshis (limited by MAX_ABET_ADDR_BALANCE_SATOSHIS)
        */
        bytes32 merkleLeaf = bytes32(abetAddr) | bytes32(rawSatoshis);
        ...
        returns (bytes memory)
    {
        ...
        if (claimParamHash == 0) {
            return abi.encodePacked(
                ...
                uint8(prefixStr.length) + ETH_ADDRESS_FIRE_LEN,
                prefixStr,
                addrStr
            );
        }

        bytes memory claimParamHashStr = new bytes(CLAIM_PARAM_HASH_FIRE_LEN);

        _FIREStringFromData(claimParamHashStr, claimParamHash, CLAIM_PARAM_HASH_BYTE_LEN);

        return abi.encodePacked(
            BITCOIN_SIG_PREFIX_LEN,
            BITCOIN_SIG_PREFIX_STR,
            uint8(prefixStr.length) + ETH_ADDRESS_FIRE_LEN + 1 + CLAIM_PARAM_HASH_FIRE_LEN,
            prefixStr,
            addrStr,
            "_",
            claimParamHashStr
    ...
    {
        addrStr = new bytes(ETH_ADDRESS_FIRE_LEN);
        _FIREStringFromData(addrStr, bytes32(bytes20(addr)), ETH_ADDRESS_BYTE_LEN);
    ...
    function _FIREStringFromData(bytes memory FIREStr, bytes32 data, uint256 dataLen)
        private
        pure
    {
        uint256 offset = 0;
        ...
        for (uint256 i = 0; i < dataLen; i++) {
            ...
            FIREStr[offset++] = FIRE_DIGITS[b >> 4];
            FIREStr[offset++] = FIRE_DIGITS[b & 0x0f];
        }
    }
...
contract UTXORedeemableToken is UTXOClaimValidation {
    /**
     * @dev PUBLIC FACING: Claim a ABET address and its Satoshi balance in ABET
     ...
     * @param rawSatoshis Raw ABET address balance in Satoshis
     * @param proof Merkle tree proof
     * @param claimToAddr Destination Eth address to credit ABET to
     ...
     * @return Total number of ABET credited, if successful
     */
    ...
    {
        /* Sanity check */
        require(rawSatoshis <= MAX_ABET_ADDR_BALANCE_SATOSHIS, "FIRE: CHK: rawSatoshis");

        /* Enforce the minimum stake time for the auto-stake from this claim */
        require(autoStakeDays >= MIN_AUTO_STAKE_DAYS, "FIRE: autoStakeDays lower than minimum");
        ...
        /* Ensure signature matches the claim message containing the Eth address and claimParamHash */
        {
            ...
            require(
                ...
                "FIRE: Signature mismatch"
            );
        }

        /* Derive ABET address from public key */
        bytes20 abetAddr = pubKeyToAbetAddress(pubKeyX, pubKeyY, claimFlags);

        /* Ensure ABET address has not yet been claimed */
        require(!abetAddressClaims[abetAddr], "FIRE: ABET address balance already claimed");

        /* Ensure ABET address is part of the Merkle tree */
        require(
            _abetAddressIsValid(abetAddr, rawSatoshis, proof),
            "FIRE: ABET address or balance unknown"
        );

        /* Mark ABET address as claimed */
        abetAddressClaims[abetAddr] = true;
    ...
    function _satoshisClaimSync(
        ...
        private
        returns (uint256 totalClaimedABET)
    {
        GlobalsCache memory g;
        GlobalsCache memory gSnapshot;
        _globalsLoad(g, gSnapshot);

        totalClaimedABET = _satoshisClaim(
            g,
            rawSatoshis,
            claimToAddr,
            abetAddr,
        ...
        return totalClaimedABET;
    }

    /**
     * @dev Credit an Eth address with the ABET value of a raw Satoshis balance
     * @param g Cache of stored globals
     * @param rawSatoshis Raw ABET address balance in Satoshis
     * @param claimToAddr Destination Eth address for the claimed ABET to be sent
     * @param abetAddr Bitcoin address (binary; no base58-check encoding)
     * @param autoStakeDays Number of days to auto-stake, subject to minimum auto-stake days
     * @param referrerAddr Eth address of referring user (optional; 0x0 for no referrer)
     * @return Total number of ABET credited, if successful
        ...
        returns (uint256 totalClaimedABET)
    {
        ...
        require(g._currentDay >= CLAIM_PHASE_START_DAY, "FIRE: Claim phase has not yet started");
        require(g._currentDay < CLAIM_PHASE_END_DAY, "FIRE: Claim phase has ended");
        ...
        /* Sanity check */
        require(
            g._claimedAbetAddrCount < CLAIMABLE_ABET_ADDR_COUNT,
            "FIRE: CHK: _claimedAbetAddrCount"
        );

        (uint256 adjSatoshis, uint256 claimedABET, uint256 claimBonusABET) = _calcClaimValues(
            g,
            rawSatoshis
        );
        ...
        totalClaimedABET = _remitBonuses(
            claimToAddr,
            abetAddr,
            claimFlags,
            rawSatoshis,
        ...
        uint256 autoStakeABET = totalClaimedABET * AUTO_STAKE_CLAIM_PERCENT / 100;
        _stakeStart(g, autoStakeABET, autoStakeDays, true);

        /* Mint remaining claimed ABET to claim address */
        _mint(claimToAddr, totalClaimedABET - autoStakeABET);
    ...
    function _remitBonuses(
        ...
        uint256 adjSatoshis,
        uint256 claimedABET,
        uint256 claimBonusABET,
        address referrerAddr
    )
        private
        returns (uint256 totalClaimedABET)
    {
        totalClaimedABET = claimedABET + claimBonusABET;

        uint256 originBonusABET = claimBonusABET;
        ...
        if (referrerAddr == address(0)) {
            ...
            /* Referral bonus of 10% of total claimed ABET to claimer */
            uint256 referralBonusABET = totalClaimedABET / 10;

            totalClaimedABET += referralBonusABET;

            /* Then a cumulative referrer bonus of 20% to referrer */
            uint256 referrerBonusABET = totalClaimedABET / 5;

            originBonusABET += referralBonusABET + referrerBonusABET;

            if (referrerAddr == claimToAddr) {
                /* Self-referred */
                totalClaimedABET += referrerBonusABET;
                _emitClaim(
                    claimToAddr,
                    abetAddr,
                    claimFlags,
                ...
                _mint(referrerAddr, referrerBonusABET);
            }
        }

        _mint(ORIGIN_ADDR, originBonusABET);
        ...
        private
    {
        emit Claim( // (auto-generated event)
            uint256(uint40(block.timestamp))
                ...
                | (uint256(uint72(claimedABET)) << 160),
    ...
    function _calcClaimValues(GlobalsCache memory g, uint256 rawSatoshis)
        private
        pure
        returns (uint256 adjSatoshis, uint256 claimedABET, uint256 claimBonusABET)
    {
        ...
        require(
            ...
            "FIRE: CHK: _claimedSatoshisTotal"
        );
        ...
        /* Convert to ABET and calculate speed bonus */
        claimedABET = adjSatoshis * ABET_PER_SATOSHI;
        claimBonusABET = _calcSpeedBonus(claimedABET, daysRemaining);

        return (adjSatoshis, claimedABET, claimBonusABET);
    }

    /**
     * @dev Apply Silly Whale adjustment
     * @param rawSatoshis Raw ABET address balance in Satoshis
     * @return Adjusted ABET address balance in Satoshis
     */
        ...
        returns (uint256)
    {
        if (rawSatoshis < 1000e8) {
            /* For < 1,000 ABET: no penalty */
            return rawSatoshis;
        }
        if (rawSatoshis >= 10000e8) {
            /* For >= 10,000 ABET: penalty is 75%, leaving 25% */
            return rawSatoshis / 4;
        }
        /*
            For 1,000 <= ABET < 10,000: penalty scales linearly from 50% to 75%

            penaltyPercent  = (abet - 1000) / (10000 - 1000) * (75 - 50) + 50
                            = (abet - 1000) / 9000 * 25 + 50
                            = (abet - 1000) / 360 + 50

            appliedPercent  = 100 - penaltyPercent
                            = 100 - ((abet - 1000) / 360 + 50)
                            = 100 - (abet - 1000) / 360 - 50
                            = 50 - (abet - 1000) / 360
                            = (18000 - (abet - 1000)) / 360
                            = (18000 - abet + 1000) / 360
            ...
            adjustedAbet     = abet * appliedPercent / 100
                            = abet * ((19000 - abet) / 360) / 100
                            = abet * (19000 - abet) / 36000

            adjustedSat     = 1e8 * adjustedAbet
                            = 1e8 * (abet * (19000 - abet) / 36000)
     ...
     * @param adjSatoshis Adjusted ABET address balance in Satoshis (after Silly Whale)
     * @param daysRemaining Number of reward days remaining in claim phase
     * @return Adjusted ABET address balance in Satoshis (after Silly Whale and Late-Claim)
     */
     ...
     * @param claimedABET ABET claimed from adjusted ABET address balance Satoshis
     * @param daysRemaining Number of claim days remaining in claim phase
     * @return Speed bonus in ABET
     */
    function _calcSpeedBonus(uint256 claimedABET, uint256 daysRemaining)
        private
        pure
        returns (uint256)
    {
        /*
            ...
            bonusABET = claimedABET  * ((daysRemaining - 1)  /  (CLAIM_PHASE_DAYS - 1)) * 20%
                        = claimedABET  * ((daysRemaining - 1)  /  (CLAIM_PHASE_DAYS - 1)) * 20/100
                        = claimedABET  * ((daysRemaining - 1)  /  (CLAIM_PHASE_DAYS - 1)) / 5
                        = claimedABET  *  (daysRemaining - 1)  / ((CLAIM_PHASE_DAYS - 1)  * 5)
        */
        return claimedABET * (daysRemaining - 1) / ((CLAIM_PHASE_DAYS - 1) * 5);
    }
}
...
contract TransformableToken is UTXORedeemableToken {
        ...
        payable
    {
        ...
        require(enterDay < CLAIM_PHASE_END_DAY, "FIRE: Lobbies have ended");

        uint256 rawAmount = msg.value;
        require(rawAmount != 0, "FIRE: Amount required");
    ...
    function xfLobbyExit(uint256 enterDay, uint256 count)
        external
    {
        require(enterDay < _currentDay(), "FIRE: Round is not complete");
        ...
        do {
            ...
            if (referrerAddr == address(0)) {
                ...
                uint256 referralBonusABET = xfAmount / 10;

                xfAmount += referralBonusABET;

                /* Then a cumulative referrer bonus of 20% to referrer */
                uint256 referrerBonusABET = xfAmount / 5;

                if (referrerAddr == msg.sender) {
                    /* Self-referred */
                    xfAmount += referrerBonusABET;
                    _emitXfLobbyExit(enterDay, headIndex, xfAmount, referrerAddr);
                } else {
                    /* Referred by different address */
                    _emitXfLobbyExit(enterDay, headIndex, xfAmount, referrerAddr);
    ...
    function xfLobbyFlush()
        external
    {
        require(address(this).balance != 0, "FIRE: No value");
        ...
        returns (uint256 rawAmount, address referrerAddr)
    {
        ...
        require(entry.rawAmount != 0, "FIRE: Param invalid");
        ...
        returns (uint256 waasLobby)
    {
        if (enterDay >= CLAIM_PHASE_START_DAY) {
            ...
            waasLobby = unclaimed * ABET_PER_SATOSHI / CLAIM_PHASE_DAYS;

            _globalsSync(g, gSnapshot);
        } else {
            waasLobby = WAAS_LOBBY_SEED_ABET;
        }
        return waasLobby;
    }
...
contract FIRE is TransformableToken {
    constructor()
        public
    {
